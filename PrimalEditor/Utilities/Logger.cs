...
namespace PrimalEditor.Utilities
{
    ...
    static class Logger
    {
        ...
        {
            await Application.Current.Dispatcher.BeginInvoke(new Action(() => // creates a new dispatch on the ui thread
            {
                _messages.Add(new LogMessage(type, msg, file, caller, line)); // ui thread finds the time slot to write a message
            }));
        }
